...
std::string TextGenerator::cleanWord(const std::string& word) {
    std::string result;
    size_t start = 0;
    while (start < word.length() && ispunct((unsigned char)word[start]) && word[start] != '\'' && word[start] != '-')
        start++;
    size_t end = word.length();
    while (end > start && ispunct((unsigned char)word[end - 1]) && word[end - 1] != '\'' && word[end - 1] != '-')
        end--;
...
void TextGenerator::addRecord(const prefix& pref, const std::string& suffix) {
...
void TextGenerator::generateText(std::ostream& output, int maxWords) {
    ...
    if (!currentPref.empty()) {
        ...
        if (!firstWord.empty())
            firstWord[0] = ::toupper((unsigned char)firstWord[0]);
        output << firstWord;
        wordCount++;
    }

    for (size_t i = 1; i < currentPref.size() && wordCount < maxWords; i++) {
    ...
    while (wordCount < maxWords) {
        ...
        if (suffix == NOPREFIX) {
            ...
            if (wordCount < maxWords) {
                ...
                if (!nextFirst.empty())
                    nextFirst[0] = ::toupper((unsigned char)nextFirst[0]);
                output << " " << nextFirst;
                wordCount++;
            }
            for (size_t i = 1; i < currentPref.size() && wordCount < maxWords; i++) {
